...
- Entre 2016 e 2017, ministramos novamente o curso e, depois de mais de 100 alunos formados, decidimos aumentar a gama e a frequência das aulas
- De 2017 para frente, decidimos nos tornar uma empresa de fato.
- Mais de 400 alunos formados!

Fernando Corrêa e Caio Lente:

- Mestrandos em estatística no IME-USP
- Trabalham com webscraping diariamente
...
1. Como **identificar** o objeto de interesse

1. Como funciona uma página HTML e como **navegar** por seus elementos

1. O protocolo HTTP e como **replicar** seu funcionamento

1. Como **validar** resultados e evitar os problemas mais comuns

1. Como **parsear** as páginas baixadas

1. Como **iterar** ao longo de muitas páginas e como salvar esses resultados


Se der tempo, veremos: 
...
```{r eval=FALSE}
# necessário
library(httr)       # navegar
library(xml2)       # parsear
library(rvest)      # navegar e parsear

# útil
library(janitor)    # arrumar dados zuados
library(abjutils)   # replicar e iterar
library(tidyverse)  # everything
library(scrapr)     # replicar e identificar
...
1. Será mais fácil analisar os dados.

--

2. Será mais fácil juntar os dados.
...
3. Use a função `furrr::future_map`.
...
httr::GET(
  ...
  httr::write_disk("static/nubank.html", overwrite = TRUE)
)
```
...
s <- webdriver::Session$new(port = pjs$port)
s$go("https://app.nubank.com.br/#/login")
s$takeScreenshot()

s$getSource() %>% 
  readr::write_file("static/nubank.html")
```
